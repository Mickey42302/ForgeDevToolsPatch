...
To install this mod, place the JAR file in your "mods" directory.

*Note: Although the mod can be used on multiplayer, some features will only work if the server has the mod installed as well. For servers that don't use Forge, the development features can be enabled by overriding the "IS_RUNNING_IN_IDE" flag to true. Depending on what server engine you are using, you may need to override the "isDevelopment" flag instead.*
